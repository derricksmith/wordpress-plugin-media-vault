...
//    mgjp_mv_fix_meta_file($attachment_id);
    $file = get_post_meta($attachment_id, '_wp_attached_file', true);
 ...
 * @param $attachment_id
 */
function mgjp_mv_fix_meta_file($attachment_id)
{
    ...
    // re-check if the file path is empty
    $file = get_post_meta($attachment_id, '_wp_attached_file', true);
    if (strlen($file) < 4 && isset($post['guid']))
    {
        $guid = str_replace($upload_dir['baseurl'] . '/', '', $post['guid']);
